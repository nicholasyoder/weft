...
## Environment/IBL lighting — **done, hemisphere-only** (2026-09-10)
...
**Environment/IBL lighting closed the ambient gap named above — see [ADR-0021](../decisions/0021-hemisphere-ambient-lighting.md).** The flat `ambient = 0.15` term is gone, replaced by a Fresnel/metallic-weighted hemisphere ambient (sky color blended with ground color by the shading normal), fit entirely inside the existing lights uniform buffer — zero new bind groups, zero new textures. Scene-authorable via a new `[environment]` table (`sky_color`/`ground_color`/`intensity`), same shape as `[audio]`. `games/sandbox`'s `playground.toml` authors one. **Deliberately not full IBL**: no cubemap, no reflections of real scene content, no roughness-aware prefiltering — a single-color-per-hemisphere approximation. Real reflective IBL is still a future item (see ADR-0021's "Revisit when"), just a lower-urgency one now that the near-black-metal-in-shadow case is fixed.

- **The lighting bind group is already at wgpu's 4-bind-group ceiling**, not just "no consumer needs more yet." ADR-0019 folded lights, the shadow map, and the shadow sampler into one group specifically because a 4th group wouldn't fit (`shader.wgsl:48-50`). The next lighting feature — a prefiltered cubemap, cascaded shadow maps, a point-light shadow cubemap — can't just add a bind group; it has to grow entries within the existing ones or restructure. Worth deciding on a storage-buffer-based light list (removes the fixed 4-light cap and gives headroom for new entries) *before* more scenes get authored assuming ≤4 lights, rather than after.
