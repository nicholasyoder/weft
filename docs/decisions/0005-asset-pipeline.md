...
- A real asset (hand-authored or generated) needs more than one mesh/primitive per file — likely the first time a non-trivial character or prop is imported. At that point `ImportedAsset` needs to become a list, and the scene-fragment emitter needs to emit multiple entities or a multi-part `MeshRef`.
