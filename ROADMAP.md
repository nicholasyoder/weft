...
- [`docs/roadmap/known-issues.md`](docs/roadmap/known-issues.md) — a *different* kind of list: not missing capabilities, but places the code doesn't do what its own docs/tests/design already claim (architectural debt, process/testing gaps). Kept trimmed to currently-open items only — fixed items are removed on the spot, `git log` is the record of what was fixed and why. Check this before starting new tier work — some items there will shape how new capability work should be built, not just get fixed independently later.
