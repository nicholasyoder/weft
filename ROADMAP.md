...
- Prototype one of ADR-0009's two asset-generation patterns against a real asset this game needs, and record what was learned. — **still open**: the first playable milestone (see below) used only hardcoded engine meshes (cube/plane/sphere), no imported asset — a real prototype of either ADR-0009 pattern is deferred to whenever this game's next milestone actually needs one.
