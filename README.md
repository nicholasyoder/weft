...
Phases 0–19 are done: a deterministic ECS core (`engine-core`), a text scene format (`engine-scene`), headless-first `wgpu` rendering with live windowed play, text/UI rendering, GPU skeletal animation, PBR materials, multi-light shading, shadow mapping, and hemisphere ambient lighting (`engine-render`), a content-addressed glTF/image/font/audio asset pipeline with multi-mesh/multi-primitive import (`engine-assets`), Lua scripting with hot-reload and a live/scripted-input API (`engine-script`), a `rapier3d` physics substrate with raycasts, sensor triggers, and a kinematic character controller (`engine-physics`), a deterministic skeletal-animation sampler (`engine-anim`), an audio playback + deterministic offline-mixdown layer (`engine-audio`), a CLI and MCP server exposing all of it (`engine-cli`, `engine-mcp`), and a first real test game (`games/sandbox` — a capsule character controller (WASD, jump, wall-sliding) collects sensor-trigger pickups with sound effects around a shadow-casting, point-lit, multi-material physics playground with a raycast-gated lever and scene-authored ambient lighting, in a custom imported font's HUD). The full build log is [`docs/roadmap/completed-phases.md`](docs/roadmap/completed-phases.md). **Tier 1 and all of Tier 2 are now closed**, as of Phase 19 (2026-09-10).

A 2026-09-01 capability audit weighed the engine against what a fully realized game — realistic graphics, audio, animation, UI, everything a shipped game needs — actually requires, closing Tier 1 and driving Tier 2's physics substrate (Phase 15) and PBR/multi-light/shadows (Phase 16) work. A follow-up 2026-09-04 audit then asked a narrower question — is that visual-realism work itself shaped so a genuinely full, high-quality-graphics game can be built on it, not just a proof that the pipeline works — and reprioritized what's still ahead accordingly, ranking multi-mesh/multi-part asset import and environment/IBL lighting above the remaining light-count/spot-light items. Phase 17 then gave `games/sandbox` its first real use of point lights, PBR roughness/metallic variation, normal mapping, and `engine.raycast`; Phase 18 closed the multi-mesh/multi-part import gap ([ADR-0020](docs/decisions/0020-multi-mesh-gltf-import.md)) — `engine import` now accepts any number of meshes/primitives per glTF file instead of just one. Phase 19 then closed the ambient-lighting gap ([ADR-0021](docs/decisions/0021-hemisphere-ambient-lighting.md)) with a Fresnel/metallic-weighted hemisphere ambient term, deliberately scoped to fit inside the existing lights uniform buffer rather than building a full cubemap/IBL pipeline — real reflective IBL stays a future item. [Tier 2](docs/roadmap/tier-2-visual-and-gameplay-realism.md)'s remaining open items are the light-count/spot-light/shadow-quality list and multi-part-asset follow-ons (a parent/child transform component, multi-part skinned animation) that no concrete consumer needs yet. [Tier 3](docs/roadmap/tier-3-polish-and-feel.md) gained a mipmapping item and a rendering-scalability section (frustum culling, `f32` world-position precision) alongside promoting the tone-mapping/HDR half of "post-processing" above the rest of that list. Ship-readiness concerns (packaging, additional input devices, CI, networking) remain the lowest-priority tier — see [ROADMAP.md](ROADMAP.md). Separately, [`docs/roadmap/known-issues.md`](docs/roadmap/known-issues.md) tracks places the code doesn't (yet) do what its own docs/tests/design already claim — architectural debt and process/testing gaps, kept trimmed to what's currently open. **The roadmap — phases, tiers, and known issues alike — is explicitly a living document**, and the tiers are a suggested order, not a queue: every boundary is a checkpoint to reconsider the plan as real constraints show up, never a fixed spec to build against on faith.
